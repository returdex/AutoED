...
it('starts independently installed target entries, runs operation and fresh-generation selfchecks, then exposes real status',async()=>{
  const h=createHarness(),parent=realpathSync(h.root),provisional={root:join(parent,'installation'),parent,excludedRoots:[] as never[]},secrets=secretStoreForProvisioning(provisional);let selection:{root:string;parent:string;excludedRoots:never[]}|undefined,installationId:string|undefined,manifest:ReturnType<ReturnType<typeof createFixtureVerifier>>|undefined,safe=false;
  try{protectPath(parent);symlinkSync(realpathSync('node_modules'),join(parent,'node_modules'),'dir');const release=await compiledRelease(parent,'B'),signed=await signSyntheticManifests(parent,[release.bytes]);manifest=createFixtureVerifier(signed.publicKey,signed.fingerprint)(release.bytes,Buffer.from(signed.signatures[0]!,'base64'),{os:'darwin',arch:'arm64',version:'26.5.2',schema:1,protocol:1});selection=provisional;const preview=createInstallPreview(manifest,selection),confirmation=confirmInstallPreview(preview,'INSTALL '+preview.scopeHash);installationId=preview.installationId;
