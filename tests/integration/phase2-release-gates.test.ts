...
function publishedAssets(release=artifactReceipt(),firstId=700){const target=(platform:'macos'|'windows',id:number)=>({id,name:release.targets[platform].name,bytes:release.targets[platform].bytes,sha256:release.targets[platform].sha256,url:release.targets[platform].url});return{macos:target('macos',firstId),windows:target('windows',firstId+1)};}
