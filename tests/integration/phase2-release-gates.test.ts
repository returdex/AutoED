...
function artifactReceipt(selected=selection(),tested=report(selected),bodies={macos:Buffer.from('macos archive'),windows:Buffer.from('windows archive')}){
  const target=(platform:'macos'|'windows')=>{const token=platform==='macos'?'darwin-arm64':'win32-x64',body=bodies[platform],name=`autoed-${selected.version}-${token}.tar.gz`;return{name,localPath:`.runtime/releases/${selected.version}/${name}`,url:`https://github.com/returdex/AutoED/releases/download/${selected.version}/${name}`,bytes:body.length,sha256:sha(body),manifestSha256:hash(platform==='macos'?'7':'8'),signatureSha256:hash(platform==='macos'?'9':'a'),capabilityClosureSha256:hash('4')};};
  return{schema:1,status:'built_signed_verified_local',owner:'returdex',repository:'returdex/AutoED',repositoryId:1350421724,version:selected.version,tag:selected.tag,commit:selected.commit,tree:selected.tree,buildId:selected.buildId,selectionSha256:canonicalSha256(selected),testReportSha256:canonicalSha256(tested),sourceSha256:selected.sourceSha256,manifestSha256:hash('5'),fingerprint:selected.trustFingerprint,license:selected.license,immutable:true,promptSha256:hash('6'),capabilitiesSha256:canonicalSha256(PHASE2_CAPABILITIES),targets:{macos:target('macos'),windows:target('windows')},gaps:selected.gaps};
}
...
it('update handoff is read-only and rejects platform spoof or identity drift',async()=>{
  ...
  await expect(verifyPhase2UpdateGate(args,deps)).resolves.toMatchObject({schema:1,status:'pass',mode:'pre-update',platform:'windows',buildId:release.buildId,resultCode:'WINDOWS_UPDATE_READY',phase3:'blocked'});expect(reads).toBe(4);
