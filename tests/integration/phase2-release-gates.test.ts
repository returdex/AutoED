...
it('two-layer install prompt signs an archive-independent core and externally binds both exact targets',()=>{
  ...
  for(const target of Object.values(release.targets)){
    expect(core).not.toContain(target.name);expect(core).not.toContain(target.url);expect(core).not.toContain(String(target.bytes));expect(core).not.toContain(target.sha256);
