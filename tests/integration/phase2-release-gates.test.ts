...
it('rehearsal attestation rejects a stale on-disk unnumbered build identity',()=>{
  ...
  const value={schema:1,status:'pass',kind:'unnumbered_release_rehearsal',releaseCoordinate:null,commit:commit('a'),tree:commit('b'),buildId:hash('c'),sourceSha256:hash('d'),managedRuntime:{verified:true,node:'24.20.0',npm:'11.19.0'},focused:{status:'pass',commandSha256:hash('1'),tests:1,skipped:0,todo:0},quality:Object.fromEntries([...['typecheck','unit','integration','ui','native'].map((name,index)=>[name,{status:'pass',commandSha256:sha(`${index}:${name}`),tests:1,skipped:0,todo:0}]),['sensitiveScan',{status:'pass',findings:0,reportSha256:hash('2')}]]),closures:{macos:{status:'pass',platform:'macos',files:1,assets:8,sensitiveFindings:0},windows:{status:'pass',platform:'windows',files:1,assets:8,sensitiveFindings:0}},prompt:{status:'pass',targetCount:2,assetCount:16,commandsBound:true,latestReferences:0},publication:{status:'pass',contractTests:1,remoteMutations:0,fullVerifierInvocations:1},failureHistory:[],completedAt:now};
