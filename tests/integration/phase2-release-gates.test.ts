...
it('pre-verifier readiness rejects wrong metadata identity, redirect or length without retrying the full verifier',async()=>{
  const release=artifactReceipt(),assets=Object.fromEntries(Object.entries(release.targets).map(([platform,target],index)=>[platform,{id:900+index,name:target.name,bytes:target.bytes,sha256:target.sha256,url:target.url}])),publication={schema:1,status:'pass',owner:'returdex',repositoryId:1350421724,version:release.version,tag:release.tag,buildId:release.buildId,manifestSha256:release.manifestSha256,immutable:true,assets,checkedAt:now},metadata=availabilityReadinessMetadata(release,publication);
