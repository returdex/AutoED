...
async function closeFixtureHost(host:ReturnType<typeof spawn>){
  if(host.exitCode!==null||host.signalCode!==null)return;
  host.stdin?.end();
  for(const signal of [null,'SIGTERM','SIGKILL'] as const){
    const deadline=Date.now()+5000;
    while(host.exitCode===null&&host.signalCode===null&&Date.now()<deadline)await pause(25);
    if(host.exitCode!==null||host.signalCode!==null)return;
    if(signal!==null&&!host.kill(signal))throw new Error('HOST_FIXTURE_CLOSE_UNCONFIRMED');
  }
  throw new Error('HOST_FIXTURE_CLOSE_UNCONFIRMED');
}
