...
it.each(['host_receipt_unknown','process_receipt_unknown','startup_ref','mixed_pins','access_denied'] as const)('returns cleanup_pending for %s',async fault=>{
...
it('blocks cleanup for a real live host lease and a controlled PID/start-identity mismatch',async()=>{
  ...
  const fixtureEntry=join(parent,'host-fixture.mjs');writeFileSync(fixtureEntry,script);const env={PATH:process.env.PATH,HOME:process.env.HOME,TMPDIR:process.env.TMPDIR},host=spawn(process.execPath,['--experimental-transform-types',fixtureEntry],{cwd:parent,stdio:['pipe','pipe','pipe'],env});try{const [ready]=await Promise.race([once(host.stdout!,'data'),once(host,'exit').then(()=>{throw new Error('HOST_FIXTURE_EXITED');})]);expect(String(ready)).toBe('ready\n');await expect(cleanupRuntimeInventory(selection)).rejects.toThrow('HOST_OWNERSHIP_UNCONFIRMED');const directory=join(selection.root,'runtime/clients'),path=join(directory,readdirSync(directory)[0]!),prior=JSON.parse(readFileSync(path,'utf8')),dummy=spawn(process.execPath,['--input-type=module','-e','setInterval(()=>{},1000)'],{stdio:'ignore',env});writeFileSync(path,JSON.stringify({...prior,pid:dummy.pid}));await expect(cleanupRuntimeInventory(selection)).rejects.toThrow('HOST_OWNERSHIP_UNCONFIRMED');writeFileSync(path,JSON.stringify(prior));dummy.kill('SIGTERM');await once(dummy,'exit');host.stdin!.end();await once(host,'exit');await cleanupRuntimeInventory(selection);expect(existsSync(directory)).toBe(false);}finally{if(host.exitCode===null){host.stdin!.end();await once(host,'exit');}await h.cleanup();}
},30000);
