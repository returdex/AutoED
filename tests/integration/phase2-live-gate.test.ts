...
import {PHASE2_BUILD_OBLIGATIONS,PHASE2_CAPABILITIES} from '../../scripts/release/phase2-gate.mjs';
...
describe('phase 2 read-only update gate',()=>{
  function updateFixture(platform:'macos'|'windows'='macos'){
    ...
    const target=(name:string)=>({name,localPath:`.runtime/releases/${version}/${name}`,url:`https://github.com/returdex/AutoED/releases/download/v${version}/${encodeURIComponent(name)}`,bytes:4096,sha256:artifactSha256,manifestSha256:'5'.repeat(64),signatureSha256:'6'.repeat(64),capabilityClosureSha256:'7'.repeat(64)}),targets={macos:target('autoed-darwin-arm64.tar.gz'),windows:target('autoed-win32-x64.zip')},artifacts={schema:1,status:'built_signed_verified_local',owner:'returdex',repository:'returdex/AutoED',repositoryId:1350421724,version,tag:`v${version}`,commit:selection.commit,tree:selection.tree,buildId,selectionSha256:sha(selection),testReportSha256:sha(tests),sourceSha256,versionSetSha256:selection.versionSetSha256,manifestSha256,fingerprint:selection.trustFingerprint,license:selection.license,immutable:true,promptSha256:sha('verified prompt'),capabilitiesSha256:sha(PHASE2_CAPABILITIES),targets,gaps:selection.gaps};
