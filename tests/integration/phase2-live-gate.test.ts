...
describe('phase 2 read-only update gate',()=>{
  function updateFixture(platform:'macos'|'windows'='macos'){
    ...
    const obligations=['auth01.sealed_source_contract','auth03.state_contract','auth03.persistence_isolation','auth04.ownership_contract','auth04.ownership_integration','sec02.fixed_operations_contract','sec02.fixed_operations_integration','uat01.distribution_contract'].map(id=>({id,status:'pass',reportDigest:sha(id)}));const tests={schema:1,status:'pass',buildId,artifactReceiptSha256:sha(artifacts),suites:{typecheck:'pass',unit:'pass',integration:'pass',ui:'pass',native:'pass'},sensitiveScan:'pass',obligations};
