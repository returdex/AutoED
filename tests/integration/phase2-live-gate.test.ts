...
describe('phase 2 native evidence producer',()=>{
  ...
  function nativeFixture(){const calls={runtime:0,checks:0,append:0,l:0,source:0,profile:0,browser:0};const reports=macIds.map(id=>({id,status:'pass',resultCode:'CHECK_PASSED',reportDigest:sha(id)}));return{calls,reports,deps:{nativePlatform:'macos',async readRuntime(){calls.runtime++;return runtime({platform:'macos'});},async runFixedChecks(){calls.checks++;return structuredClone(reports);},async appendNativeBundle(value:unknown){calls.append++;return{schema:1,status:'pass',resultCode:'NATIVE_EVIDENCE_RECORDED',platform:'macos',version,buildId,generation:7,bundleId:'10000000-0000-4000-8000-000000000111',obligations:macIds,gaps:[],value};}}};}
  it('produces one current-build platform-bound S/I/N bundle and is idempotent without L authority',async()=>{const f=nativeFixture(),args=['--produce','--runtime','current','--platform','macos'];const first=await runPhase2NativeEvidence(args,f.deps),second=await runPhase2NativeEvidence(args,f.deps);expect(first).toMatchObject({status:'pass',platform:'macos',buildId,obligations:macIds});expect(second).toMatchObject({status:'pass',bundleId:first.bundleId});expect(f.calls).toMatchObject({append:2,l:0,source:0,profile:0,browser:0});expect(first.obligations).not.toContain(expect.stringMatching(/\.L$|live/i));});
  it('rejects platform mismatch, incomplete/unrequired/L reports and retains failure without pass append',async()=>{for(const mutate of[(f:ReturnType<typeof nativeFixture>)=>{f.deps.nativePlatform='windows';},(f:ReturnType<typeof nativeFixture>)=>{f.reports.pop();},(f:ReturnType<typeof nativeFixture>)=>{f.reports[0]={...f.reports[0]!,id:'arbitrary.operation'};},(f:ReturnType<typeof nativeFixture>)=>{f.reports[0]={...f.reports[0]!,id:'a.login.L'};},(f:ReturnType<typeof nativeFixture>)=>{f.reports[0]={...f.reports[0]!,status:'fail',resultCode:'CHECK_FAILED'};}]){const f=nativeFixture();mutate(f);await expect(runPhase2NativeEvidence(['--produce','--runtime','current','--platform','macos'],f.deps)).rejects.toThrow(/^NATIVE_EVIDENCE_/);expect(f.calls.append).toBe(0);}const f=nativeFixture();await expect(runPhase2NativeEvidence(['--produce','--runtime','current','--platform','windows'],f.deps)).rejects.toThrow('NATIVE_EVIDENCE_PLATFORM_MISMATCH');expect(f.calls.runtime).toBe(0);});

  it('persists one transactional server-derived bundle idempotently and never touches the L ledger',async()=>{const root=protectedRoot(),db=openDatabase(join(root,'native.sqlite'));try{const binding={platform:'macos' as const,version,buildId,artifactSha256,manifestSha256,generation:7,checkedAt:observedAt},store=new SQLiteNativeEvidenceStore(db,{now:()=>now}),service=new NativeEvidenceService(store,{current:async()=>binding},{expectedGeneration:7}),checks=macIds.map(id=>({id,status:'pass' as const,resultCode:'CHECK_PASSED',reportDigest:sha(id)})),command={schema:1,suiteDigest:sha(checks),checks};const first=await service.record(command),second=await service.record(command);expect(first).toMatchObject({status:'pass',platform:'macos',buildId,generation:7,obligations:macIds});expect(second.bundleId).toBe(first.bundleId);expect((db.prepare('SELECT count(*) AS n FROM phase2_build_obligations').get() as {n:number}).n).toBe(11);expect((db.prepare("SELECT count(*) AS n FROM uat_receipts WHERE evidence='L'").get() as {n:number}).n).toBe(0);const failed=await service.record({...command,suiteDigest:'e'.repeat(64),checks:checks.map((item,index)=>index===0?{...item,status:'fail' as const,resultCode:'CHECK_FAILED'}:item)});expect(failed).toMatchObject({status:'fail',resultCode:'NATIVE_EVIDENCE_CHECK_FAILED'});expect((db.prepare('SELECT count(*) AS n FROM phase2_build_obligations').get() as {n:number}).n).toBe(11);expect((db.prepare("SELECT count(*) AS n FROM phase2_native_runs WHERE status='fail'").get() as {n:number}).n).toBe(1);}finally{db.close();}});
});
