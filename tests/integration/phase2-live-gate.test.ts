...
describe('phase 2 read-only update gate',()=>{
  function updateFixture(platform:'macos'|'windows'='macos'){
    ...
    const asset=(name:string,role?:string)=>({name,localPath:`.runtime/releases/${version}/${name}`,url:`https://github.com/returdex/AutoED/releases/download/v${version}/${encodeURIComponent(name)}`,bytes:4096,sha256:artifactSha256,...(role?{role}:{})}),target=(token:string,extension:string)=>{const name=`autoed-${version}-${token}.${extension}`,updater={bootstrap:asset(`autoed-${version}-${token}-bootstrap.${token==='darwin-arm64'?'sh':'ps1'}`),manifest:asset(`autoed-${version}-${token}-manifest.json`),signature:asset(`autoed-${version}-${token}-manifest.sig`),artifacts:['installer','program','node','browser'].map(role=>asset(`autoed-${version}-${token}-${role}.tar.gz`,role))};return{...asset(name),manifestSha256:'5'.repeat(64),signatureSha256:'6'.repeat(64),capabilityClosureSha256:'7'.repeat(64),updater};},targets={macos:target('darwin-arm64','tar.gz'),windows:target('win32-x64','zip')},core=renderPhase2InstallPromptCore(selection,tests),draft={schema:1,status:'built_signed_verified_local',owner:'returdex',repository:'returdex/AutoED',repositoryId:1350421724,version,tag:`v${version}`,commit:selection.commit,tree:selection.tree,buildId,selectionSha256:sha(selection),testReportSha256:sha(tests),sourceSha256,versionSetSha256:selection.versionSetSha256,manifestSha256,fingerprint:selection.trustFingerprint,license:selection.license,immutable:true,installPromptCoreSha256:sha(core),externalPromptSha256:'8'.repeat(64),capabilitiesSha256:sha(PHASE2_CAPABILITIES),targets,gaps:selection.gaps},prompt=renderPhase2ExternalInstallPrompt(draft),artifacts={...draft,externalPromptSha256:sha(prompt)};
    let assetId=101;const targetAssets=(value:any)=>[value,value.updater.bootstrap,value.updater.manifest,value.updater.signature,...value.updater.artifacts],assets=Object.fromEntries(Object.entries(targets).map(([key,value])=>[key,targetAssets(value).map(item=>({id:assetId++,name:item.name,bytes:item.bytes,sha256:item.sha256,url:item.url}))])),publication={schema:1,status:'pass',owner:'returdex',repositoryId:1350421724,version,tag:`v${version}`,buildId,manifestSha256,immutable:true,assets,checkedAt:observedAt};
    const availability={schema:1,status:'pass',anonymous:true,immutable:true,version,tag:`v${version}`,buildId,manifestSha256,assets,checkedAt:observedAt},freshAvailability={...availability,checkedAt:new Date(now-60_000).toISOString()};
