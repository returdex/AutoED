...
it('requests host reload before creating a journal or entering maintenance',async()=>{
  ...
  const {writeFileSync}=await import('node:fs');writeFileSync(entry,script);const host=spawn(process.execPath,['--experimental-transform-types',entry],{cwd:fixture.parent,stdio:['pipe','pipe','pipe'],env:{PATH:process.env.PATH,HOME:process.env.HOME,TMPDIR:process.env.TMPDIR,AUTOED_SYNTHETIC_TEST:'1',AUTOED_SYNTHETIC_PORT:process.env.AUTOED_SYNTHETIC_PORT}});
