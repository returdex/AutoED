...
import {zipEntries,validateLinkGraph,renderBootstrapPayload,verifyBootstrapManifest} from '../../packages/installer/src/archive-core.js';
...
it('actual shell boot verifies Node, embedded code, signature and installer before execution; NODE_OPTIONS cannot inject code',async()=>{
  const h=createHarness();try{const root=realpathSync(h.root);protectPath(root);const compiled=join(root,'compiled');execFileSync(process.execPath,['node_modules/typescript/bin/tsc','--outDir',compiled],{timeout:30000,stdio:'pipe'});
    ...
    const marker=join(root,'executed'),injected=join(root,'injected'),entry='packages/installer/src/install.js',program=`require('node:fs').appendFileSync(${JSON.stringify(marker)},'verified')`,archive=tar([{path:entry,data:program}]);
    ...
    for(const scenario of ['valid','node','core','signature','installer']){
      ...
      const script=join(root,scenario+'.sh');writeFileSync(script,source);let code=0;try{execFileSync('/bin/sh',[script,'--staging-parent',parent],{env:{...process.env,PATH:'/usr/bin:/bin',NODE_OPTIONS:`--require=${join(root,'inject.cjs')}`},timeout:30000,stdio:'pipe'});}catch(error){code=(error as {status:number}).status??1;}
