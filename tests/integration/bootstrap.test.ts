...
it('two-stage synthetic signer binds post-discovery bytes to the same ephemeral fingerprint and exits',async()=>{
  const h=createHarness();try{const root=realpathSync(h.root),bytes=Buffer.from('post-key-discovery');const signed:any=await withSyntheticSigner(root,async(signer:any)=>({fingerprint:signer.fingerprint,signature:(await signer.sign([bytes]))[0]}));expect(signed.signerExited).toBe(true);expect(signed.result.fingerprint).toBe(signed.fingerprint);expect(verify(null,bytes,createPublicKey(signed.publicKey),Buffer.from(signed.result.signature,'base64'))).toBe(true);await expect(withSyntheticSigner(root,async()=>{throw new Error('callback');})).rejects.toThrow('SYNTHETIC_SIGNER_FAILED');await expect(withSyntheticSigner(root,async(signer:any)=>signer.sign([Buffer.alloc(8*1024*1024+1)]))).rejects.toThrow('INVALID_SYNTHETIC_INPUT');await expect(withSyntheticSigner(root,async()=>await new Promise(()=>{}),{timeoutMs:25})).rejects.toThrow('SYNTHETIC_SIGNER_FAILED');expect(readdirSync(root)).toEqual([]);await expect(withSyntheticSigner(root as any,null as any)).rejects.toThrow('INVALID_SYNTHETIC_INPUT');}finally{await h.cleanup();}
});
