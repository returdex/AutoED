...
it('scans every reachable source-history blob and blocks a secret deleted from the working tree',()=>{
  const root=realpathSync(mkdtempSync(join(tmpdir(),'autoed-release-history-'))),secret='ghp_'+'A'.repeat(36);try{execFileSync('git',['init','-q'],{cwd:root});execFileSync('git',['config','user.name','returdex'],{cwd:root});execFileSync('git',['config','user.email','123+returdex@users.noreply.github.com'],{cwd:root});writeFileSync(join(root,'private.txt'),secret);execFileSync('git',['add','private.txt'],{cwd:root});execFileSync('git',['commit','-qm','old secret'],{cwd:root});rmSync(join(root,'private.txt'));writeFileSync(join(root,'safe.txt'),'safe');execFileSync('git',['add','-A'],{cwd:root});execFileSync('git',['commit','-qm','remove secret'],{cwd:root});expect(()=>scanReachableHistory(root,'HEAD')).toThrow('SOURCE_HISTORY_REJECTED');try{scanReachableHistory(root,'HEAD');}catch(error){expect(String(error)).not.toContain(secret);}}finally{rmSync(root,{recursive:true,force:true});}
});

it('creates no publish action without a Plan 13 receipt and requires anonymous full-byte availability',async()=>{
  expect(()=>createPublishPlan({preflight:{status:'pass'},approvalReceipt:null,version:'0.1.0-beta.1',assets:[]})).toThrow('PUBLISH_APPROVAL_REQUIRED');const body=Buffer.from('asset'),asset={name:'program.tar.gz',url:'https://github.com/returdex/AutoED/releases/download/0.1.0-beta.1/program.tar.gz',bytes:body.length,sha256:sha(body)};await expect(verifyPublicAvailability({version:'0.1.0-beta.1',assets:[asset],fetch:async()=>({status:404,arrayBuffer:async()=>new ArrayBuffer(0),headers:new Map()})})).rejects.toThrow('PUBLIC_AVAILABILITY_FAILED');expect(await verifyPublicAvailability({version:'0.1.0-beta.1',assets:[asset],fetch:async(_url,options)=>{expect(options?.headers?.authorization).toBeUndefined();return{status:200,arrayBuffer:async()=>body,headers:new Map([['content-length',String(body.length)]])};}})).toMatchObject({status:'pass',version:'0.1.0-beta.1'});
});
