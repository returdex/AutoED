...
describe('legacy installation ownership recovery',()=>{
  ...
  it('fails closed on non-device drift, credential/process evidence and proof interruption',async()=>{
    ...
    await expect(prepareInstallationIdentityRecovery(fixture.selection,fixture.verify,fixture.secrets)).rejects.toThrow('INSTALLATION_RECOVERY_UNCONFIRMED');writeFileSync(activePath,activeBytes);protectPath(activePath);
    ...
    await expect(prepareInstallationIdentityRecovery(fixture.selection,fixture.verify,fixture.secrets)).rejects.toThrow('INSTALLATION_RECOVERY_UNCONFIRMED');writeFileSync(launcherPath,launcherBytes);protectPath(launcherPath);
    ...
    await expect(prepareInstallationIdentityRecovery(fixture.selection,fixture.verify,fixture.secrets)).rejects.toThrow('INSTALLATION_RECOVERY_UNCONFIRMED');writeFileSync(signaturePath,signatureBytes);protectPath(signaturePath);
    ...
    await expect(prepareInstallationIdentityRecovery(fixture.selection,fixture.verify,fixture.secrets)).rejects.toThrow('INSTALLATION_RECOVERY_UNCONFIRMED');writeFileSync(closurePath,closureBytes);protectPath(closurePath);
    const lock=join(fixture.selection.root,'installer-staging','update.lock');mkdirSync(lock,{mode:0o700});protectPath(lock);
    await expect(prepareInstallationIdentityRecovery(fixture.selection,fixture.verify,fixture.secrets)).rejects.toThrow('INSTALLATION_RECOVERY_UNCONFIRMED');rmdirSync(lock);
    ...
    await expect(prepareInstallationIdentityRecovery(fixture.selection,fixture.verify,fixture.secrets)).rejects.toThrow('INSTALLATION_RECOVERY_UNCONFIRMED');
    await fixture.secrets.set(legacy.installationId,'installer',token!);
    const processPath=join(fixture.selection.root,'runtime','api.json'),savedProcess=readFileSync(processPath),record=JSON.parse(savedProcess.toString());
    ...
    await expect(prepareInstallationIdentityRecovery(fixture.selection,fixture.verify,fixture.secrets)).rejects.toThrow('INSTALLATION_RECOVERY_UNCONFIRMED');unlinkSync(clientPath);
