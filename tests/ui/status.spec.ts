...
test('real browser pairs via HTTP, refreshes without jobs, clears forbidden data, and marks network snapshot stale',async({page,context,browser})=>{
  ...
  try{
    ...
    f.deny(false);await page.getByRole('button',{name:'刷新状态'}).click();await expect(page.locator('#protected')).toContainText(f.build.version);
    f.sessions.revokeAll();await page.getByRole('button',{name:'刷新状态'}).click();await expect(page.locator('#protected')).toBeEmpty();
...
for(const [result,stage,cleanup,actual,message,previousInstallation] of [
  ['failed','stopped','not_observed',false,'安装失败，服务尚未就绪。当前没有可恢复的旧版','none'],
  ['failed','stopped','not_observed',false,'旧版本与恢复状态尚未确认','unknown'],
  ['failed','stopped','not_observed',false,'旧版本与恢复状态尚未确认','present'],
  ['running','cleanup','cleanup_pending',true,'旧受管程序、入口或进程尚未清理完成','present'],
  ['restored','rollback','complete',true,'升级失败，已恢复旧版。','present'],
  ['human_needed','stopped','pending',false,'操作已停止，尚不能确认安全恢复方式。','unknown'],
  ['running','selfcheck','pending',true,'检测到组件版本不一致','present'],
] as const)test(`actual status projection ${result}/${stage}/${cleanup}/${previousInstallation}`,async({page})=>{
  const h=createHarness();let f:Awaited<ReturnType<typeof createStatusFixture>>|undefined;
  try{
    const assets=join(h.root,'assets');await buildStatusAssets(join(process.cwd(),'apps/status'),assets);f=await createStatusFixture({assetsRoot:assets});
    ...
    await f.projections.writeInstall({operationId,result,stage,cleanup,previousInstallation,targetBuild:{...f.build,version:'0.1.0-beta.2'},actualBuild:actual?f.build:null,checkedAt:new Date().toISOString()},{operationId,expectedGeneration:gate.generation});
