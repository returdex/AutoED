...
6. Run the selected bootstrap exactly once. Any nonzero, interrupted, uncertain, or human-needed result stops the attempt; do not rerun the same bootstrap or infer that an absent preview means only missing interactive input.
...
8. Run the managed prompt-driven update, reload the Codex MCP integration when instructed, and require readiness plus cleanup=complete. 不得覆盖已发布版本，不得强制降级，不得删除课程资料。
9. Do not open a school source or perform official login/MFA during installation. 不得登录，不得读取、请求、记录或返回密码、MFA、输入值或按键。
10. The dedicated Profile is sensitive credential storage. 不得复制或备份 Profile，不得导出 Cookie、storage state、密钥或原始网络捕获。
11. Publication, installation, update, synthetic, integration, or native checks do not create live evidence. Windows remains not_run/human_needed, live remains not_run/human_needed, and Phase 3 remains blocked until the required hard human gates pass.
