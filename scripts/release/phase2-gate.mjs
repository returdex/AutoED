...
1. Run this prompt only in a same-host/account local projectless Codex task. A task attached to any saved project, repository or worktree must stop with UPDATE_TASK_CONTEXT_INVALID before downloading or invoking the updater. The updater task must not read, edit, commit, or push the current task workspace.
2. Select only the native operating-system target declared by the external prompt. Windows evidence must come from native Windows; WSL, Linux, JavaScript parity, or macOS evidence cannot substitute for it.
3. Download the complete archive without credentials, ranges, fallback mirrors, or a caller-supplied URL. Verify the exact byte count and SHA-256 from the external prompt before opening it.
4. Verify the fixed Ed25519 fingerprint, signed manifest, signature, capability closure, this core member, unchanged PolyForm Noncommercial 1.0.0 license, dependency closure, target identity, and every declared member before installation.
5. Refuse missing, extra, duplicate, linked, private, sensitive, stale, mismatched, unsigned, alternate-key, or partially downloaded content. Do not accept a substituted external prompt as this signed core.
6. Before the selected bootstrap runs, establish exactly one live interactive PTY/session whose stdin, stdout and stderr are TTYs. The updater rejects a non-interactive transport with INTERACTIVE_SESSION_REQUIRED before recovery or installation mutation. Do not use a detached command, non-interactive shell, closed stdin, transcript replay, or a second session for either confirmation.
7. Run the selected bootstrap exactly once and keep that same process/session alive through both human gates. When it emits a recovery or install preview, surface the complete preview, pause for the user's real exact confirmation, and send only that confirmation plus one newline to the same still-running PTY/session. Any nonzero, interrupted, uncertain, malformed, or human-needed result stops the attempt; do not rerun the bootstrap or infer that an absent preview means only missing interactive input.
...
9. Run the managed prompt-driven update, reload the Codex MCP integration when instructed, and require readiness plus cleanup=complete. 不得覆盖已发布版本，不得强制降级，不得删除课程资料。
10. Do not open a school source or perform official login/MFA during installation. 不得登录，不得读取、请求、记录或返回密码、MFA、输入值或按键。
11. The dedicated Profile is sensitive credential storage. 不得复制或备份 Profile，不得导出 Cookie、storage state、密钥或原始网络捕获。
12. Publication, installation, update, synthetic, integration, or native checks do not create live evidence. Windows remains not_run/human_needed, live remains not_run/human_needed, and Phase 3 remains blocked until the required hard human gates pass.
