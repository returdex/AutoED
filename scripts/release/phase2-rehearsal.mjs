...
export async function exercisePhase2PublicationContract({identity,assembly,now,commandLedger=/** @type {string[]} */([]),availabilityDeps={}}){
  try{
    ...
    const remoteMutations=commandLedger.filter(entry=>PUBLICATION_FORBIDDEN.test(entry)).length;if(commandLedger.length!==0||remoteMutations!==0)runnerFail('PRE_RUNNER','PUBLICATION_SIDE_EFFECT');completed.add('no_side_effect');
