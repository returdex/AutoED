...
import {classifyPhase2RehearsalFailure,phase2RehearsalCommandSha256,reportPhase2RehearsalCommand} from './phase2-rehearsal-reporter.mjs';
...
async function runFixedCommand(root,id,scanner,ledger){
  ...
  for(const step of spec.steps){let child,failure;try{child=await runPhase2Detached({program:node,args:['scripts/dev/runtime.mjs',...step.args],cwd:root,timeoutMs:spec.ceiling*1000,scanner});}catch(error){failure=error;}
    let reclaimed;try{reclaimed=await runPhase2Detached({program:node,args:['scripts/release/reclaim-synthetic-processes.mjs'],cwd:root,timeoutMs:15000,scanner});}catch{runnerFail('PRE_RUNNER',`SYNTHETIC_PROCESS_RECLAIM_FAILED_AT_${step.name.toUpperCase().replaceAll('-','_')}`);}if(reclaimed.exitCode!==0||reclaimed.signal)runnerFail('PRE_RUNNER',`SYNTHETIC_PROCESS_RECLAIM_FAILED_AT_${step.name.toUpperCase().replaceAll('-','_')}`);
    if(failure){const match=/^PHASE2_REHEARSAL_FAILED class=PRE_RUNNER code=([A-Z0-9_]{1,64})$/.exec(failure?.message??'');if(match)runnerFail('PRE_RUNNER',`${match[1]}_AT_${step.name.toUpperCase().replaceAll('-','_')}`);throw failure;}if(child.exitCode!==0||child.signal){let category;try{category=classifyPhase2RehearsalFailure({runner:step.runner,exitCode:child.exitCode,signal:child.signal,stdout:child.stdout});}catch{category='PROCESS_EXIT_NONZERO';}runnerFail('PRE_SOURCE',phase2StepFailureCode(`COMMAND_${category}`,step.name));}let report;try{report=reportPhase2RehearsalCommand({runner:step.runner,exitCode:child.exitCode,signal:child.signal,stdout:child.stdout,commandSha256});}catch{runnerFail('PRE_SOURCE',phase2StepFailureCode('COMMAND_REPORT_INVALID',step.name));}passed+=report.passed;}
