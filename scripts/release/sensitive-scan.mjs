...
function privatePathDenied(path){if(PRIVATE_PATH.test(path))return true;const segments=path.split('/');for(let index=0;index<segments.length;index++){if(!/^cookies?$/i.test(segments[index]))continue;const dependencyPackage=segments[index-1]==='node_modules'||(index>1&&segments[index-2]==='node_modules'&&segments[index-1].startsWith('@'));if(!dependencyPackage)return true;}return false;}
