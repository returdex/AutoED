...
export function discoverClientEndpoint(selection:RootSelection){
  ...
  const record=read();
  const expectedEntry=realpathSync(fileURLToPath(new URL('../../../apps/api/src/main.js',import.meta.url)));
  if(record.installationId!==metadata.installationId||record.role!=='api'||record.controlPort!==metadata.port||record.entrypoint!==expectedEntry||record.executable!==realpathSync(process.execPath))throw new Error('IDENTITY_MISMATCH');
  return {installationId:metadata.installationId,scope:metadata.approvedScope,port:metadata.port,
