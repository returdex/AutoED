...
const metadataSchema = z.strictObject({
  schema: z.literal(1), installationId: z.uuid(), port: z.number().int().min(1).max(65535), rootAlias: z.literal('managed-root'),
...
export async function initializeInstallation(selection: RootSelection, store: SecretStore = new NativeSecretStore(), installationId: string = randomUUID()): Promise<InstallationMetadata> {
  ...
  const metadata = metadataSchema.parse({ schema: 1, installationId, port: endpoint.port, rootAlias: 'managed-root', ...endpoint.syntheticTest ? { syntheticTest: true } : {}, ownership, approvedScope: scope, credentials });
