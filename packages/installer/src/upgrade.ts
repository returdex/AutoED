...
function rollbackPins(selection:RootSelection,operationId:string){let root:string,record:z.infer<typeof Pins>;try{root=assertManagedPath(managedPaths(selection.root),`installer-staging/activation-${z.uuid().parse(operationId)}`);record=Pins.parse(JSON.parse(readFileSync(join(root,'pins.json'),'utf8')));}catch{throw new Error('ACTIVATION_REALITY_UNCONFIRMED');}if(record.operationId!==operationId||record.installationId!==readInstallation(selection).installationId)throw new Error('ACTIVATION_REALITY_UNCONFIRMED');return{root,record};}
/** Recognizes only the two complete, hash-pinned realities used by rollback and rollback resumption. */
export function inspectRollbackActivation(selection:RootSelection,operationId:string){
  const {root,record}=rollbackPins(selection,operationId),currentBin=join(selection.root,'bin'),currentActive=join(selection.root,'active.json'),oldBin=join(root,'old-bin'),oldActive=join(root,'old-active.json'),failedBin=join(root,'failed-bin'),failedActive=join(root,'failed-active.json'),names=readdirSync(root).sort();
  if(existsSync(oldBin)&&existsSync(oldActive)&&!existsSync(failedBin)&&!existsSync(failedActive)){if(names.some(name=>!['old-active.json','old-bin','pins.json'].includes(name)))throw new Error('ACTIVATION_REALITY_UNCONFIRMED');validatePinSide(oldBin,oldActive,record.old);validatePinSide(currentBin,currentActive,record.candidate);return{root,state:'candidate_active' as const,oldActive,targetActive:currentActive};}
  if(!existsSync(oldBin)&&!existsSync(oldActive)&&existsSync(failedBin)&&existsSync(failedActive)){if(names.some(name=>!['failed-active.json','failed-bin','failed-jobs.sqlite','pins.json'].includes(name)&&!/^failed-recovery-[0-9a-f-]{36}\.sqlite$/.test(name)))throw new Error('ACTIVATION_REALITY_UNCONFIRMED');validatePinSide(currentBin,currentActive,record.old);validatePinSide(failedBin,failedActive,record.candidate);return{root,state:'old_active' as const,oldActive:currentActive,targetActive:failedActive};}
  throw new Error('ACTIVATION_REALITY_UNCONFIRMED');
}
...
export function recoverCandidateActivation(selection:RootSelection,operationId:string){
  const {root,record}=rollbackPins(selection,operationId);
...
/** Stop the generation through its authenticated control route before opening
 * the maintenance gate. Passive waiting for a stale heartbeat made the
 * reopen boundary timing-dependent on macOS.
 */
...
export async function upgradeConfirmed(preview:InstallPreview,confirmation:InstallConfirmation,options:UpgradeOptions){
  ...
  try{
    ...
    await step('migrated',async()=>{if(db.pragma('user_version',{simple:true})!==5||db.pragma('integrity_check',{simple:true})!=='ok')throw new Error('SCHEMA_INCOMPATIBLE');});
    ...
    await step('reopened',async()=>{for(const identity of supervisor!.registered().reverse())await awaitGenerationExit(supervisor!,identity);client=undefined;await new SQLiteMaintenanceStore(db).exitMaintenance(operationId,gate.generation);await j.clientAdmission('normal_probe');supervisor=runtimeSupervisor(selection,manifest);await supervisor.start({installationId:metadata.installationId,role:'api',build:preview.target});await supervisor.start({installationId:metadata.installationId,role:'worker',build:preview.target});client=verifiedInstallerClient(selection,manifest);});
