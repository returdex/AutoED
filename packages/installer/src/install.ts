...
import {prepareInterruptedUpgradeRecovery,confirmInterruptedUpgradeRecovery,recoverUpgrade,resumeCleanupUpgrade,RecoveryError} from './recovery.js';
import {confirmInstallationIdentityRecovery,prepareInstallationIdentityRecovery} from './ownership-recovery.js';
export async function installConfirmed(preview:InstallPreview,confirmation:InstallConfirmation,options:{store?:SecretStore;archives:Record<string,Buffer>}){
  ...
  approvedManifest(preview,confirmation);await initializeInstallation(selection,options.store??secretStoreForProvisioning(selection),preview.installationId);
...
interface InstallerDependencies {verify?:typeof verifyRelease;store?:SecretStore;acquire?:(manifest:VerifiedManifest,stagingParent:string)=>Promise<Record<string,Buffer>>;execute?:typeof upgradeConfirmed;recover?:typeof recoverUpgrade;resumeCleanup?:typeof resumeCleanupUpgrade;interactive?:()=>boolean}
...
export async function runInstallerCLI(dependencies:InstallerDependencies={},args=process.argv.slice(2)){
  if(!(dependencies.interactive??(()=>process.stdin.isTTY===true&&process.stdout.isTTY===true&&process.stderr.isTTY===true))())throw new Error('INTERACTIVE_SESSION_REQUIRED');
  ...
  const lines=createInterface({input:process.stdin,output:process.stderr});try{
    const root=args[6]??await lines.question(`安装根目录候选：${defaultRoot()}。请输入要预览的完整目录；空白取消：\n`);if(!root)throw new Error('CONFIRMATION_REQUIRED');const selection={root,parent:dirname(root),excludedRoots:[]};if(existsSync(root)){
      try{readInstallation(selection);}catch(error){
        if(!(error instanceof Error)||error.message!=='INVALID_INSTALLATION')throw error;
        const recovery=await prepareInstallationIdentityRecovery(selection,dependencies.verify??verifyRelease,dependencies.store??secretStoreForProvisioning(selection));process.stdout.write(JSON.stringify({type:'installation_identity_recovery_preview',recovery})+'\n');
        const answer=await lines.question(`仅当上述恢复检查全部通过且范围正确时，输入 RECOVER ${recovery.scopeHash} 迁移旧版安装身份；其他输入取消：\n`);await confirmInstallationIdentityRecovery(recovery,answer);process.stdout.write(JSON.stringify({type:'installation_identity_recovered',status:'complete',schema:2,reason:recovery.reason})+'\n');
      }
      const recovery=await prepareInterruptedUpgradeRecovery(selection,dependencies.verify??verifyRelease,dependencies.store??secretStoreForInstallation(selection));if(recovery){process.stdout.write(JSON.stringify({type:'interrupted_upgrade_recovery_preview',recovery})+'\n');const verb=recovery.action==='continue'?'CONTINUE':'ROLLBACK',answer=await lines.question(`检测到签名绑定的未完成更新。仅输入 ${verb} ${recovery.scopeHash} 执行所示恢复动作；其他输入取消：\n`),result=await confirmInterruptedUpgradeRecovery(recovery,answer),recovered={type:'install_recovery' as const,...result};if('build'in result&&assertOwnedLaunchers(selection).manifestHash===manifest.manifestHash){const complete={type:'install_result' as const,...result};process.stdout.write(JSON.stringify(complete)+'\n');return complete;}process.stdout.write(JSON.stringify(recovered)+'\n');}
    }const preview=createInstallPreview(manifest,selection);process.stdout.write(JSON.stringify({type:'install_preview',preview})+'\n');
    ...
    process.stdout.write(JSON.stringify({type:'install_stage',stage:'artifact_acquire',phase:'intent'})+'\n');const archives=await acquire(manifest,stagingParent);process.stdout.write(JSON.stringify({type:'install_stage',stage:'artifact_acquire',phase:'done'})+'\n');let oldManifest:VerifiedManifest|undefined;if(preview.previousInstallation==='present'){const active=assertOwnedLaunchers(preview.selection),paths=managedPaths(preview.selection.root),base=`installer-staging/manifests/${active.build.buildId}`;oldManifest=(dependencies.verify??verifyRelease)(protectedInput(assertManagedPath(paths,base+'/manifest.json'),8*1024*1024),protectedInput(assertManagedPath(paths,base+'/manifest.sig'),64),{os:platform.platform as 'darwin'|'win32',arch:platform.arch as 'arm64'|'x64',version:platform.version,schema:1,protocol:1});if(oldManifest.manifestHash!==active.manifestHash)throw new Error('OLD_ARTIFACT_UNVERIFIED');}
    const execute=dependencies.execute??upgradeConfirmed,result=await execute(preview,confirmation,{archives,...(oldManifest?{oldManifest}:{}),...(dependencies.store?{store:dependencies.store}:{}),progress:(stage,phase)=>process.stdout.write(JSON.stringify({type:'install_stage',stage,phase})+'\n'),cleanup:async context=>context.oldManifest?cleanupManaged({selection:context.selection,operationId:context.journal.header.operationId,currentManifest:context.manifest,oldManifest:context.oldManifest}):cleanupRuntimeInventory(context.selection).then(()=>({complete:true})).catch(()=>({complete:false,code:'CLEANUP_PENDING'}))});process.stdout.write(JSON.stringify({type:'install_result',...result})+'\n');return result;
  }finally{lines.close();}
}
if(process.argv[1]&&fileURLToPath(import.meta.url)===process.argv[1])void runInstallerCLI().catch(error=>{const code=error instanceof RecoveryError&&/^[A-Z_]+$/.test(error.causeCode)?`${error.code}_${error.causeCode}`:error instanceof Error&&/^[A-Z_]+$/.test(error.message)?error.message:'INSTALLATION_FAILED';process.stderr.write(JSON.stringify({type:'install_error',code})+'\n');process.exitCode=1;});
