...
export async function runInstallerCLI(dependencies:InstallerDependencies={},args=process.argv.slice(2)){
  ...
  const lines=createInterface({input:process.stdin,output:process.stderr});try{
    ...
    if(preview.previousInstallation==='present'){const operationId=pendingCleanupRecovery(preview.selection);if(operationId){const result=await (dependencies.resumeCleanup??resumeCleanupUpgrade)(preview.selection,operationId,{verify:dependencies.verify??verifyRelease,secrets:dependencies.store??new NativeSecretStore()}),recovered={type:'install_recovery' as const,...result};if(assertOwnedLaunchers(preview.selection).manifestHash===manifest.manifestHash){const complete={type:'install_result' as const,...result};process.stdout.write(JSON.stringify(complete)+'\n');return complete;}process.stdout.write(JSON.stringify(recovered)+'\n');}}
