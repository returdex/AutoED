...
import {pendingCleanupRecovery,recoverUpgrade,resumeCleanupUpgrade,RecoveryError} from './recovery.js';
...
export async function runInstallerCLI(dependencies:InstallerDependencies={},args=process.argv.slice(2)){
  ...
  const lines=createInterface({input:process.stdin,output:process.stderr});try{
    const root=args[6]??await lines.question(`安装根目录候选：${defaultRoot()}。请输入要预览的完整目录；空白取消：\n`);if(!root)throw new Error('CONFIRMATION_REQUIRED');const selection={root,parent:dirname(root),excludedRoots:[]};if(existsSync(root)){
      ...
      const operationId=pendingCleanupRecovery(selection);if(operationId){const result=await (dependencies.resumeCleanup??resumeCleanupUpgrade)(selection,operationId,{verify:dependencies.verify??verifyRelease,secrets:dependencies.store??secretStoreForInstallation(selection)}),recovered={type:'install_recovery' as const,...result};if(assertOwnedLaunchers(selection).manifestHash===manifest.manifestHash){const complete={type:'install_result' as const,...result};process.stdout.write(JSON.stringify(complete)+'\n');return complete;}process.stdout.write(JSON.stringify(recovered)+'\n');}
