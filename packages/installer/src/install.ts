...
interface InstallerDependencies {verify?:typeof verifyRelease;store?:SecretStore;acquire?:(manifest:VerifiedManifest,stagingParent:string)=>Promise<Record<string,Buffer>>}
...
export async function runInstallerCLI(dependencies:InstallerDependencies={},args=process.argv.slice(2)){
  ...
  const lines=createInterface({input:process.stdin,output:process.stderr});try{
    ...
    if(preview.previousInstallation==='present')throw new Error('UPGRADE_ENGINE_REQUIRED');
    ...
    const archives=await acquire(manifest,stagingParent),result=await installConfirmed(preview,confirmation,{...(dependencies.store?{store:dependencies.store}:{}),archives});process.stdout.write(JSON.stringify({type:'install_result',...result})+'\n');return result;
  }finally{lines.close();}
}
