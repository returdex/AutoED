...
const Inactive=z.strictObject({schema:z.literal(1),installationId:z.uuid(),operationId:z.uuid(),build:z.object({buildId:hash}),manifestHash:hash,state:z.literal('inactive_rollback_artifact')});
...
// Activation pins are created by upgrade.hashFile with the same 1 MiB bound.
// Metadata remains on regular()'s 64 KiB default; only already-pinned launcher files use this limit.
const sha=(path:string)=>createHash('sha256').update(regular(path,1024*1024)).digest('hex');
...
export async function cleanupManaged(context:CleanupContext){
  try{
    ...
    const inactive=join(activation,'inactive.json');if(existsSync(inactive)){const record=Inactive.parse(JSON.parse(regular(inactive).toString()));if(record.installationId!==active.installationId||record.operationId!==context.operationId||record.build.buildId!==context.oldManifest.manifest.build.buildId||record.manifestHash!==context.oldManifest.manifestHash||existsSync(join(activation,'old-active.json'))||existsSync(join(activation,'old-bin')))throw new Error('MIXED_PINS');await cleanupRuntimeInventory(context.selection);return {complete:true as const,code:'CLEANUP_COMPLETE' as const};}
