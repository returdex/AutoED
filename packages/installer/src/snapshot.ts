...
function databaseDigest(path:string){const fd=openSync(path,'r');try{const stat=fstatSync(fd);if(!stat.isFile()||stat.nlink!==1||stat.size<1||stat.size>8*1024**3)throw new Error('SNAPSHOT_SIZE_LIMIT');const hash=createHash('sha256'),chunk=Buffer.alloc(1024*1024);let position=0;while(position<stat.size){const n=readSync(fd,chunk,0,Math.min(chunk.length,stat.size-position),position);if(n===0)throw new Error('SNAPSHOT_INVALID');hash.update(chunk.subarray(0,n));position+=n;}const after=fstatSync(fd);if(after.size!==stat.size||after.mtimeMs!==stat.mtimeMs)throw new Error('SNAPSHOT_INVALID');return {databaseHash:hash.digest('hex'),databaseBytes:stat.size};}finally{closeSync(fd);}}
...
export async function createSnapshot(selection:RootSelection,db:Database.Database,context:{operationId:string;generation:number}):Promise<Snapshot>{
  ...
  mkdirSync(root,{mode:0o700});protectPath(root);const databasePath=join(root,'snapshot.sqlite'),fd=openSync(databasePath,'wx',0o600);closeSync(fd);protectPath(databasePath);const owned=lstatSync(databasePath),deadline=Date.now()+30000;
  await db.backup(databasePath,{progress:()=>{if(Date.now()>deadline||state(db,context.operationId,context.generation)!==writeGeneration)throw new Error('SNAPSHOT_FENCE_CHANGED');return 128;}});
