...
function renderSourceCard(source:AuthSource) {
  ...
  rows(card,[['官方来源',source.officialOrigin],['auth',explained('auth',source.auth)],['capability',explained('capability',source.capability)],['health',explained('health',source.health)],['freshness',explained('freshness',source.freshness)],['completeness',explained('completeness',source.completeness)],['完整显示名',source.identity?.displayName??'未提供；不代表身份已确认。'],['完整学校邮箱',source.identity?.schoolEmail??'未提供；不代表身份已确认。'],['共享专属 Profile',profile],['最近检查时间 / 证据',`${source.checkedAt??unknown} / 受保护状态投影；页面不推断 L`],['指定课程',source.identity?.selectedCourseName??'未提供；不读取课程内容。'],['课程可见性',source.courseAccess==='allowed'?'已确认可见':'未确认'],['result code / next action',`${source.resultCode} / ${actionLabel(authSnapshot)}`]]);
  return card;
}
...
function render(stale=false) {if(!runtimeSnapshot)return;const detailsOpen=protectedView.querySelector('details')?.open??false;protectedView.replaceChildren();pairing.hidden=true;renderOverallGate(authSnapshot,stale);renderPrivacyNotice(authSnapshot);renderSourceCards(authSnapshot);renderBindingPanel(authSnapshot);renderLoginActionPanel(authSnapshot,stale);renderCheckpointLedger(stale);renderPlatformGaps();renderExistingDiagnostics(runtimeSnapshot,stale);const details=protectedView.querySelector('details');if(details)details.open=detailsOpen;}
function clearProtected() {runtimeSnapshot=null;authSnapshot=null;job=null;receipts=[];readAt='';csrf=null;protectedView.replaceChildren();pairing.hidden=false;}
...
async function refresh() {
  if(busy)return;busy=true;refreshButton.disabled=true;announce('正在读取本地服务状态…');
  try{
    ...
    const authResponse=await request('/api/auth/status');let nextAuth:AuthSnapshot|null=null;let nextReceipts:Receipt[]=[];if(authResponse.status===401||authResponse.status===403)throw new Error('AUTH_REVOKED');if(authResponse.ok){nextAuth=await authResponse.json();nextReceipts=await readReceipts();}
  ...
  }catch(error){if(error instanceof Error&&error.message==='AUTH_REVOKED'){clearProtected();announce('此页面尚未获得本地访问权限');await beginPairing();}else if(runtimeSnapshot){render(true);announce('以下为上次读取结果，当前状态尚未确认。读取时间：'+readAt);}else announce('无法连接本地 API。请在 Codex 中使用本安装的启动或诊断步骤。');}finally{busy=false;refreshButton.disabled=false;}
}
