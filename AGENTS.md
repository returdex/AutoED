...
The real Phase 2 Plan 02-14 macOS update attempt for `0.1.0-beta.31` failed with `ENTRYPOINT_MISMATCH`: the active beta.19 API/Worker remained healthy, but the build and entrypoints did not transition, cleanup and paired UI were not observed, and no school access began. Beta.31 is therefore immutable published-but-invalidated history for this gate; never retry, overwrite, delete or relabel it. The release-to-installation contract has been repaired and the required post-beta.31 unnumbered R0/R1 rehearsal passed, but no later beta has been assigned. The current stop is candidate assignment under `.planning/RELEASE-STABILIZATION.md` R2. This repository task remains the non-updating controller: do not perform the update, click OS approval, restart Codex, create a pass receipt, run 02-14 Task 3, begin source configuration/login or advance to 02-15 until a later candidate completes R2–R5 and the user returns a genuine successful update result. Phase 1 remains partial, Windows remains `not_run / human_needed`, all real L evidence remains pending and Phase 3 remains blocked.
