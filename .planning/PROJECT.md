...
- 01-12硬门禁已由用户明确完成；01-13随后以隔离`returdex`身份创建公开`returdex/AutoED`并发布不可覆盖`0.1.0-beta.1/.2`。两个版本全部资产已匿名重新获取并验签/hash/闭包通过。全局账号未切换；Windows原生与用户更新UAT仍未运行，必须停在01-14。
