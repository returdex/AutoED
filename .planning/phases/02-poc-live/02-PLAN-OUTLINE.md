...
Beta.31's completed 02-38→02-39→02-13 chain is historical, but its real 02-14 macOS update failed with `ENTRYPOINT_MISMATCH`; it is no longer an active checkpoint input and must remain immutable. The repair completed `.planning/RELEASE-STABILIZATION.md` R0/R1 without reserving another beta. The corrective run resumes at revised 02-38 R2 candidate lock, then exact-candidate R3, 02-39 assembly/signing, and 02-13 publication/availability before 02-14 can be attempted again. Runner/transient and human-environment failures follow their separate policy classes and do not automatically create a release.
